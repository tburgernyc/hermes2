...
ALTER TABLE "outreach_campaigns" ADD CONSTRAINT "outreach_ai_match_score_range" CHECK ("outreach_campaigns"."ai_match_score" IS NULL OR ("outreach_campaigns"."ai_match_score" >= 1 AND "outreach_campaigns"."ai_match_score" <= 100));--> statement-breakpoint
ALTER TABLE "outreach_campaigns" ADD CONSTRAINT "outreach_ai_capability_match_range" CHECK ("outreach_campaigns"."ai_capability_match" IS NULL OR ("outreach_campaigns"."ai_capability_match" >= 0 AND "outreach_campaigns"."ai_capability_match" <= 1));--> statement-breakpoint
