 ...
 * in every environment. Proves the per-NAICS query the live ingest issues against api.sam.gov is
 * well-formed: the REQUIRED postedFrom/postedTo window (MM/dd/yyyy, ≤ the configured days), the SINGLE
 * `ncode` (SAM rejects a comma-joined list with zero results — ingest loops per NAICS), the fixed limit +
 * api_key, and that a set-aside filter is included only when configured.
 */
...
describe("buildSamSearchUrl", () => {
  ...
  it("sets a SINGLE ncode (never a comma list) plus the api_key + limit", () => {
    const p = paramsOf(buildSamSearchUrl("541512", KEY, { now: NOW, windowDays: 7 }));
    expect(p.get("ncode")).toBe("541512");
    expect(p.get("ncode")).not.toContain(","); // SAM's ncode is single-valued
